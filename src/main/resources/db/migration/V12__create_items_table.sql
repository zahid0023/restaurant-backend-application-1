...
$$
    ...
    BEGIN
        ...
        INSERT INTO items (unit_id, code, sort_order, created_by, updated_by)
        SELECT u.id, v.code, v.sort_order, sys_id, sys_id
        FROM units u
                 JOIN (VALUES
                     ...
                     ) v(unit_code, code, sort_order) ON u.code = v.unit_code
        WHERE NOT EXISTS (SELECT 1 FROM items i2 WHERE i2.code = v.code);
        ...
        FROM items i
                 JOIN (VALUES
                           ...
                           ('CUTLERY_SET',    'bn', 'কাটলারি সেট',         'চামচ, কাঁটা ও ছুরির সেট')

                     ) v(item_code, locale_code, name, description) ON i.code = v.item_code
